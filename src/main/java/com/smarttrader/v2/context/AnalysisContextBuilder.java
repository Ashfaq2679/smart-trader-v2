...
/**
 * Builds an AnalysisContext from a candle series ("Context Builder" in the pipeline:
 * Market Data -> Context Builder -> MarketRegimeDetector -> ... per
 * SmartTrader_V2_Production_Spec.md section 2). This didn't exist anywhere in the
 * codebase before; TradingScheduler needs it to turn raw candles into something
 * TradeEngine can consume.
 *
 * Indicator math (EMA/ATR/support-resistance/consolidation/breakout) is a reasonable,
 * standard implementation rather than something the spec prescribes exactly - flagging
 * this as the one place in the pipeline that's an interpretation, not a direct
 * transcription of a spec rule.
 */
@Component
public class AnalysisContextBuilder {
    ...
    /**
     * @param candles ascending-sorted by timestamp (CandleCacheService's output already
     *                satisfies this), at least MIN_CANDLES long
     * @param now     wall-clock time, used to compute dataLatencyMs
     */
    public AnalysisContext build(List<Candle> candles, Instant now) {
        ...
        return AnalysisContext.builder()
                ...
                .lastCandleCloseTime(last.timestamp())
                .dataLatencyMs(Math.max(0, Duration.between(last.timestamp(), now).toMillis()))
                .build();
    }
    ...
    private boolean isVolumeSpike(List<Candle> candles) {
        List<Candle> window = lastN(candles, TradingConstants.VOLUME_SPIKE_LOOKBACK + 1);
        if (window.size() < 2) {
            return false;
        }
        ...
        return avgVolume > 0 && currentVolume > avgVolume * TradingConstants.VOLUME_SPIKE_MULTIPLIER;
    }
    ...
    private boolean isStrongCandle(Candle candle) {
        ...
        return body / range >= TradingConstants.STRONG_CANDLE_BODY_RATIO;
    }
    ...
    private boolean isAtrSpike(Candle last, List<Candle> candles, double atr) {
        ...
        return trueRange(last, previous) > atr * TradingConstants.ATR_SPIKE_MULTIPLIER;
    }
